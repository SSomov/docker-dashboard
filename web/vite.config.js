				...
				files.forEach((file) => {
					...
					if (stats.isFile()) {
						...
						if (file.endsWith(".css")) {
							...
							let scriptRegex = new RegExp(
								`<script[^>]*src=["']\\.?/?assets/${escapedFile}["'][^>]*>\\s*<\\/script>`,
								"gi",
							);
							...
							if (found) {
								...
								if (remainingTags > 0) {
									console.warn(
										`Warning: ${remainingTags} script tags with src still remain for ${file}`,
									);
								}
							} else {
